...
public class MultiSelectionComboBox : ListBox
{
    ...
    /// <summary>
    /// This method can be used to trigger automatic text selection by the users code.
    /// </summary>
    ...
    private void KeyDownHandler(KeyEventArgs e)
    {
        switch (e.Key)
        {
            ...
            case Key.F4:
                ...
                // if the drop down is open, we want to move the focus to the drop down
                if (IsDropDownOpen && PART_ItemsPresenter is not null)
                {
                    Dispatcher.UIThread.Post(() =>
                        PART_ItemsPresenter.GetVisualDescendants()
                            .OfType<MultiSelectionComboBoxItem>()
                            .FirstOrDefault()?
                            .Focus(NavigationMethod.Tab), DispatcherPriority.Loaded);
                }
                // if the drop down is closed and the control is editable, we want to move the focus to the editable text box
                else if (IsEditable)
                {
                ...
                {
                    PART_SelectedItemsPresenter?.Focus(NavigationMethod.Tab);
                }
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs e)
    {
        ...
        if (e.Property == OrderSelectedItemsByProperty || e.Property == SelectedItemsProperty)
        {
            UpdateDisplaySelectedItems();
            UpdateEditableText();
        }
